		...
		{#if oeuvres.length > 0}
			{#each oeuvres as oeuvre}
				...
				>
					...
					<div
						class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 to-transparent text-white p-4 transition-opacity duration-300 opacity-100 hover:opacity-0"
					>
						...
						<p class="text-md mt-1 drop-shadow-md">
							{oeuvre.data.vendue ? 'Vendue' : 'Disponible'}
						</p>
					</div>
				</div>
			{/each}
